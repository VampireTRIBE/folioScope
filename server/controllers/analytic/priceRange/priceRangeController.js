...
module.exports.groupPriceRange = async (req, res, next) => {
  try {
    ...
    if (!groupId) throw new customError("Security Id Required", 400);
    ...
    if (range === "W") {
      ...
      const priceRange = await read_GroupPriceRange(groupId, oneWeekAgo);
      return res.status(200).json({
        success: true,
    ...
    if (range === "M") {
      ...
      const priceRange = await read_GroupPriceRange(groupId, oneMonthAgo);
      return res.status(200).json({
        success: true,
    ...
    if (range === "Y") {
      ...
      const priceRange = await read_GroupPriceRange(groupId, oneYearAgo);
      return res.status(200).json({
        success: true,
    ...
    if (range === "3Y") {
      ...
      const priceRange = await read_GroupPriceRange(groupId, threeYearsAgo);
      return res.status(200).json({
        success: true,
    ...
    if (range === "MAX") {
      const priceRange = await read_GroupPriceRange(groupId);
      return res.status(200).json({
        success: true,
