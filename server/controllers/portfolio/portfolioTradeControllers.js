...
// 🔴 SNAPSHOT RECOMPUTE
// =====================================================
const recomputeSnapshot = async (assetId, session) => {
  const result = await FifoLotModel.aggregate([
    {
      $match: {
        financialAssetId: new mongoose.Types.ObjectId(assetId),
        remainingQty: { $gt: 0 },
      },
    },
    {
  ...
  if (result.length > 0) {
    ...
    investmentValue = result[0].investmentValue;
  }
...
module.exports.tradeTransaction = async (req, res) => {
  ...
  try {
    ...
    if (type === "buy") {
      ...
      await recomputeSnapshot(asset._id, session);
    }
    ...
    if (type === "sell") {
      ...
      await FinantialAssetModel.updateOne(
        ...
        {
          $inc: {
            "snapshot.realizedGain": totalSTCG + totalLTCG,
            "snapshot.STCG": totalSTCG,
            "snapshot.LTCG": totalLTCG,
          },
        },
        { session },
      );
    ...
    if (type === "dividend") {
      ...
      await FinantialAssetModel.updateOne(
        ...
        {
          $inc: {
            "snapshot.realizedGain": dividendAmount,
            "snapshot.dividendGain": dividendAmount,
          },
        },
        { session },
      );
    ...
    return res.status(201).json({ success: "Transaction completed" });
  } catch (error) {
